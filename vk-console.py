...
def message_input():
	...
	global message_text
	message_text= str(input("Сообщение (exit чтобы выйти) >> ")) # ~ Считываем текст сообщения
...
def users_ids():
	...
	for i in range(len(vk.messages.getConversations().get("items"))):
		type = vk.messages.getConversations().get("items")[i].get("conversation").get("peer").get("type")
		if type == "chat":
			chat_name = vk.messages.getConversations().get("items")[i].get("conversation").get("chat_settings").get("title")
			chat_id = vk.messages.getConversations().get("items")[i].get("conversation").get("peer").get("id")
			conversations.update({chat_name: chat_id})
		elif type == "group":
			continue
		else:
			user_chat_id = vk.messages.getConversations().get("items")[i].get("conversation").get("peer").get("id")
			username = vk.users.get(user_ids = user_chat_id)[0].get("first_name") +" "+vk.users.get(user_ids = user_chat_id)[0].get("last_name")
...
def message_get():
	global t
	t = vk.messages.getHistory(count = 20, peer_id = personal_id, rev=0) # переменная содержащая историю диалога
